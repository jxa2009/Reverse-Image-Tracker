...
async def on_reaction_add(reaction,user):
    ...
    if 'url' in attachmentsDict:
        links = ReverseImage.amazonLookup(attachmentsDict['url'])
        i = 0
        while i< len(links):
            if i<2: #prints only 2 messages so that it doesn't spam the channel
                msg = await client.send_message(channel, 'https://www.amazon.com/'+ links[i])
...
async def on_reaction_remove(reaction,user):
    ...
    if reaction.message in messageCache:
        for x in messageCache[reaction.message]:
            ...
            print("Successfully removed a message: " + x)
...
async def amazon(context):
    await client.send_message(context.message.channel, "You can upload a picture and add a reaction to the picture"+
                                                       " if you want their amazon link to appear."+
                              "remove the reaction if you want it to disappear")
